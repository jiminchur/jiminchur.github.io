...
> 이전에 나는 jekyll로 Github Blog을 운영 중이였다. 테마들이 맘에 들지 않아서 기본 테마에서 여러가지 수정을 했지만 그래도 맘에 들지 않아서 구글링 하던중 Gatsby를 이용하기로 했다. 여러 블로그 글들을 읽어가며 내가 직접 정리한 사용방법이다. 마음껏 사용하시고 안되는 거 있으면 issue에 남겨주세요

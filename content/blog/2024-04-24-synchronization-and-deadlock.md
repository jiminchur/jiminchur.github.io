...
현대 운영체제에서는 수많은 프로세스와 스레드가 동시에 실행됩니다. 이들이 하나의 자원(파일, 변수 등)에 동시에 접근하면 데이터가 꼬이거나 예상치 못한 결과가 발생할 수 있습니다. 이를 해결하기 위한 '동기화'와, 동기화 과정에서 발생할 수 있는 '교착 상태'를 정리했습니다.
...
---
동기화와 교착 상태는 멀티 스레드 프로그래밍에서 가장 까다로우면서도 중요한 주제입니다. 자바의 `synchronized`나 고(Go)의 `channel` 등 현대 언어들이 제공하는 동기화 도구들도 결국 이러한 운영체제의 기초 원리에 기반하고 있습니다.
